...
func (zmq *ZeroMQ) connect() {
	...
	socket := zmq4.NewDealer(
		context.Background(),
		zmq4.WithID(id),
		zmq4.WithDialerRetry(time.Second*time.Duration(common.VpeDialerRetry)),
		zmq4.WithDialerTimeout(time.Second*time.Duration(common.VpeDialerTimeout)),
	)
	//defer socket.Close()
	for {
		...
		if err == nil {
			...
			log.Printf("Can not connect to %s: %s, retrying.", common.GetControllerURL(), err)
		}
	}
}
...
func (zmq *ZeroMQ) runHeartbeat() {
	...
	ticker := time.NewTicker(time.Second * common.VpeHeartBeatDuration)
	for {
		select {
		case <-zmq.ctx.Done():
	...
	NormalLoop:
		for {
			...
			case <-zmq.ctx.Done():
				...
				break NormalLoop
			default:
				tchan := time.After(time.Second * 1)
				<-tchan
			}
		}
	}
